...
//bool isLostLine = false;
//bool isPreparingStop = false;
...
//float outsideProcessDiffer = 0;
//float insideProcessDiffer = 0;
...
int main(void) {
    ...
    while (1) {
        ...
        /*
        outsideProcessDiffer = OutsideLeft.Process - OutsideRight.Process;
        insideProcessDiffer = InsideLeft.Process - InsideRight.Process;
        */
        processDiffer = (sqrtf((float) OutsideLeft.Process) - sqrtf((float) OutsideRight.Process) +
                         sqrtf((float) InsideLeft.Process) -
                         sqrtf((float) InsideRight.Process)) /
                        (sqrtf((float) OutsideLeft.Process) + sqrtf((float) OutsideRight.Process) +
                         sqrtf((float) InsideLeft.Process) +
                         sqrtf((float) InsideRight.Process));

        /*isLostLine = (OutsideLeft.Process + OutsideRight.Process < 0.25);
        if (isLostLine) {
        processDiffer = 0.9F * insideProcessDiffer + 0.1F * outsideProcessDiffer;
    } else {
        processDiffer = 0.2F * insideProcessDiffer + 0.8F * outsideProcessDiffer;
    }*/

        // processDiffer = outsideProcessDiffer;
        ...
        FTM_PWM_Duty(FTM2, FTM_CH1, 77 + (int) (angleRate * 8));

        currentSpeedOutput = ExpectSpeedOutput;
        if (currentSpeedOutput > 1000) currentSpeedOutput = 1000;
        if (currentSpeedOutput < -1000) currentSpeedOutput = -1000;
